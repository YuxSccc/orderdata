...
class SLOrdersSignalCalculator(BarCalculator):
    def __init__(self, bars: list[FootprintBar], delta_threshold: float, bar_count_threshold: int, price_change_ratio_threshold: float):
        super().__init__()
        self.bars = bars
    ...
    def calc_signal(self) -> list[Signal]:
        for idx in range(len(self.bars)):
            ...
            for j in range(start_idx, end_idx + 1):
                priceList.append({"high": self.bars[j].high, "low": self.bars[j].low, "idx": j})
            ...
            if priceList[0]["idx"] == idx and priceList[0]["high"] > priceList[1]["high"] * (1 + self.price_change_ratio_threshold):
                ...
                if len(SL_orders_price) > 0:
                    ...
                    signal.set_additional_info([SL_orders_size])
